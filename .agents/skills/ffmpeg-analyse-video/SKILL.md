...
If the video is clearly a slide presentation, do **not** prefer scene detection. Instead:

1. Sample at a low fixed rate, usually `1 fps` or `2 fps`
2. Compare consecutive samples with a tiny perceptual hash
3. Group consecutive near-identical frames into stable runs
4. Keep only runs that last at least `x` seconds, usually `3-5s`
5. Use the midpoint frame of each surviving run as the slide candidate

This removes white transition frames, fade frames, and mid-animation states much better than `select='gt(scene,...)'`.

Practical rules:
- Crop to the central slide canvas before hashing when browser chrome or sidebars are visible
- Reject near-blank frames with very low variance or almost-all-white / almost-all-black luma
- Prefer stability over sensitivity; for decks, a missed micro-transition is better than noisy candidates
